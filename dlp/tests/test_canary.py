...
class TestCanaryEngine(unittest.TestCase):
    ...
    def test_inject_into_context_text_key(self):
        ...
        self.assertIn("Note: Session reference", injected[0]["text"])
        # Original key is preserved; no _canary fallback used
        self.assertNotIn("_canary", injected[0])
    ...
    def test_inject_into_context_body_key(self):
        ...
        injected, token, label = self.engine.inject_into_context(docs)
        self.assertIn("Note: Session reference", injected[0]["body"])
        self.assertNotIn("_canary", injected[0])
    ...
    def test_inject_into_context_page_content_key(self):
        ...
        injected, token, label = self.engine.inject_into_context(docs)
        self.assertIn("Note: Session reference", injected[0]["page_content"])
        self.assertNotIn("_canary", injected[0])

    def test_inject_into_context_unknown_key_falls_back(self):
        """Documents with no configured key must fall back to '_canary'."""
        docs = [{"unknown_key": "some value"}]
        injected, token, label = self.engine.inject_into_context(docs)
        self.assertIn("_canary", injected[0])
        self.assertNotIn("Note: Session reference", injected[0].get("unknown_key", ""))
